...
export function startBatch({ moldId, sources, focus, audioPath = null }) {
  ...
  processJob(jobId, mold, outputDir, focus, audioPath); // nao espera (roda em background)
  return jobId;
}

async function processJob(jobId, mold, outputDir, focus, audioPath) {
  ...
  async function worker() {
    while (index < items.length) {
      ...
      try {
        await renderOne(item.source_path, mold, outPath, focus, audioPath);
